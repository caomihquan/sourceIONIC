...
export default class ComboboxDialogForOT extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            ListEmployeeOutSite : this.props.ListEmployeeOutSite ? this.props.ListEmployeeOutSite : [] // dùng để bỏ check khi mở lại dialog
    ...
    onShow() {
         //When event Show & IsClient then set search text = '' and load data client
            // reload danh sách nv dc chọn nếu xóa bên ngoài
            let ListSelected = [...this.state.ListSelected];
            let ListEmployeeOutSite = [...this.props.ListEmployeeOutSite]
            let lstEmpSelectedCurr = ListSelected.filter((elem) => ListEmployeeOutSite.find(({ EmployeeCode }) => elem.EmployeeCode === EmployeeCode));
    
           let datacurr = [...this.state.Data];
               if(ListEmployeeOutSite.length > 0)
               {
                   for(let i=0; i< datacurr.length > 0; i++)
                   {
                       if(ListEmployeeOutSite.filter(n=>n.EmployeeCode === datacurr[i].EmployeeCode).length == 0)
                       {
                           datacurr[i].IsChecked = false;
                       }
                   }
               }
              // console.log('GGGGG', datacurr)
            this.setState({
                ListSelected:lstEmpSelectedCurr,
                Data: datacurr
            },()=>{
               if (this.state.IsClient) {
                   this.setState({
    ...
    eventOnCheckedComboboxItem = (item, index) => async() => {
        let data = this.state.Data;
        let dataFull;
        ...
        await this.a(data,dataFull,listSelected);
        
    }
    a = (data,dataFull,listSelected)=>{
        this.setState({
            Data: data,
            DataFull: dataFull,
            ListSelected: listSelected
