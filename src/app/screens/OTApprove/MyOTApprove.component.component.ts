...
@Component({
  ...
  templateUrl: './MyOTApprove.component.html',
  styleUrls: ['./MyOTApprove.component.scss'],
})
export class OTApproveComponent implements OnInit {
  ...
  loadDataRequest(): Observable<any>{
    return this.api.post(API_OT_APPROVE.Alias_GetOTApprove, API_OT_APPROVE.GetOTApprove,
        {
  ...
  eventOnDetailRequest = (item) =>{
    ...
    this.router.navigate([`OTApprove/${CommonConst.SCENE.DetailOTApprove}`],{ state: {...params} });
  };
}
