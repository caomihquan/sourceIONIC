...
@Component({
  ...
  templateUrl: './MyExtraDayOffApprove.component.html',
  styleUrls: ['./MyExtraDayOffApprove.component.scss'],
})
export class MyExtraDayOffApproveComponent implements OnInit {
  user: any;
  itemFunction: any;
  OrderByItems = [
  ...
  loadDataRequest(): Observable<any>{
    return this.api.post(HrmAPIConst.EXTRADAYOFF_APPROVE.Alias_GetLeaveApprove, HrmAPIConst.EXTRADAYOFF_APPROVE.GetLeaveApprove,
        {
        ...
        .pipe(map(result => {
            if (!result.Error) {
              ...
              const currentData = data[LeaveAndOTConst.LeaveApprove] || [];
  ...
  eventOnDetailRequest = (item) =>{
    ...
    this.router.navigate([`ExtraDayOffApprove/${CommonConst.SCENE.DetailExtraDayOffApprove}`],{ state: {...params} });
  };
}
