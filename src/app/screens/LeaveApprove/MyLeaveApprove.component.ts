...
@Component({
  ...
  templateUrl: './MyLeaveApprove.component.html',
  styleUrls: ['./MyLeaveApprove.component.scss'],
})
export class LeaveApproveComponent implements OnInit {
  ...
  loadDataRequest(): Observable<any>{
    return this.api.post(HrmAPIConst.LEAVE_APPROVE.Alias_GetLeaveApprove, HrmAPIConst.LEAVE_APPROVE.GetLeaveApprove,
        {
  ...
  eventOnDetailRequest = (item) =>{
    ...
    this.router.navigate([`LeaveApprove/${CommonConst.SCENE.DetailLeaveApprove}`],{ state: {...params} });
  };
}
